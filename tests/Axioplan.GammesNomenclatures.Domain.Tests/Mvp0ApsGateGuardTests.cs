...
public sealed class Mvp0ApsGateGuardTests
{
    ...
    private sealed class FakeMvp0Repo(IReadOnlyList<Mvp0CampaignDto> campaigns) : IMvp0Repository
    {
        ...
        public Task<IReadOnlyList<(string SheetName, string Csv)>> ConvertExcelToCsvSheetsAsync(byte[] fileContent, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyList<(string, string)>>([]);
